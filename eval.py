...
class Florence2Evaluator:
    def __init__(self, config):
        self.config = config
        self.device = config.device
        self.model, self.processor = self.load_lora_adapter_model(config.base_model, config.checkpoint, config)
    ...
    def load_lora_adapter_model(self, base_model_path, checkpoint_path, config):
        """Load model with LoRA adapters (not merged)"""
        ...
        # Wrap in LoRA model
        model = LoraModel(base_model, lora_config).to(self.device)
        ...
        processor = AutoProcessor.from_pretrained(
            processor_path if os.path.exists(os.path.join(processor_path, "tokenizer_config.json")) else base_model_path,
            trust_remote_code=True, 
            revision='refs/pr/6'
        )
    ...
    def run_inference_with_logits(self, image, ground_truth_text, task_prompt="<OCR>"):
        ...
        with torch.no_grad():
            ...
            if hasattr(self.model, 'lora_model'):  # LoRA wrapped model
            ...
            else:  # Regular model (merged weights)
    ...
    def evaluate_dataset(self, dataset_name: str, num_samples = 100) -> Tuple[float, float]:
        ...
        stream_dataset = load_dataset(dataset_name, cache_dir="./im2latex_new", split="train", streaming=True)
        dataset = datasets.Dataset.from_list(list(islice(stream_dataset, num_samples)))
        dataset.save_to_disk("./im2latex_new")
...
if __name__ == "__main__":
    ...
    evaluator = Florence2Evaluator(config)
    ...
    import csv
    # Save results to CSV
    output_csv_path = f"eval_{config.num_samples}_samples.csv"
